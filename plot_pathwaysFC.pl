...
# Generate a plot Fold change and points colour-coded by their pathways
...
my ($file, $sample1, $sample2, $output_file, $r_script, $plot_name, $flag1, $option1, $flag2, $option2) = @ARGV;
...
if (not defined $r_script) {
	print "Output R script file name not provided.\n", Usage();
	exit();
}

if (not defined $plot_name) {
	print "Output plot file name not provided.\n", Usage();
	exit();
}
...
foreach my $diff (keys %diff_data) {
	...
	while (my @row = $sth->fetchrow_array) {
		next if $last_path eq $row[7] && $last_gene = $row[2];
		...
		print $fh "$row[7]\t$row[2]\t$log2fc\tdiff\t$pval\n";
